...
greatest_month <- filter(dep_delay_by_month, average_delay == max(average_delay)) %>% 
  select(month)
...
greatest_dest <- group_by(flights, dest) %>% 
  summarise(mean_asd = mean(arr_delay, na.rm = TRUE)) %>% 
  arrange(desc(mean_asd)) %>% 
  select()
